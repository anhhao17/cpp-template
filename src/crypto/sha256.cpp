...
Status Sha256::Update(ConstByteSpan data) {
    if (finalized_) return Fail(Category, BadState, "Update after Final");

    const uint8_t* p = data.data();
    size_t         n = data.size();
    bitlen_ += static_cast<uint64_t>(n) * 8;

    // Drain any buffered partial block first.
    if (buffered_ > 0) {
        while (n > 0 && buffered_ < 64) { buffer_[buffered_++] = *p++; --n; }
        if (buffered_ == 64) { Compress(buffer_); buffered_ = 0; }
    }
    while (n >= 64) { Compress(p); p += 64; n -= 64; }
    while (n > 0) { buffer_[buffered_++] = *p++; --n; }
    return Ok();
}

Result<Digest256> Sha256::Final() {
    if (finalized_) return Fail(Category, BadState, "already finalized");

    const uint64_t bits = bitlen_;
    uint8_t        pad  = 0x80;
    Update(ConstByteSpan{&pad, 1});  // padding does not change finalized_

    uint8_t zero = 0;
    while (buffered_ != 56) {
        Update(ConstByteSpan{&zero, 1});
    }
    ...
    // Feed length manually (Update would re-extend bitlen_), then compress.
    for (int i = 0; i < 8; ++i) buffer_[buffered_++] = lenbuf[i];
    Compress(buffer_);
    buffered_ = 0;
...
Digest256 Sha256::Hash(ConstByteSpan data) {
    Sha256 h;
    h.Update(data);
